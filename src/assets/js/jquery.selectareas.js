...
(function($) {
    $.imageArea = function(parent, id) {
        ...
        $outline = $("<div class=\"select-areas-outline boxColoring"+id+" pointerEve fullArea"+id+"\" />")
            .css({
        ...
        $selection = $("<div />")
            .addClass("select-areas-background-area sectionArea"+id+" boxColoring"+id+" pointerEve fullArea"+id+"")
            .css({
        ...
        if (options.allowResize) {
            $.each(["nw", "n", "ne", "e", "se", "s", "sw", "w"], function (key, card) {
                $resizeHandlers[card] =  $("<div class=\"select-areas-resize-handler " + card + " handlerCol"+id+" pointerEve fullArea"+id+" \"/>")
                    .css({
                        opacity : 0.5,
                        position : "absolute",
                        cursor : card + "-resize"
        ...
        if (options.allowDelete) {
            ...
            $btDelete = bindToDelete($("<div class=\"delete-area pointerEve fullArea"+id+" delArea"+id+"\" />"))
    ...
    $.imageSelectAreas.prototype.init = function (object, customOptions) {
        ...
        this.$trigger = $("<div class='editMode' />")
            .css({
